        ...
        <div className="w-[90%] md:w-[80%] h-[90%] md:h-[80%] relative top-17.5 items-center grid grid-cols-1 grid-rows-3 lg:grid-cols-2 gap-x-5">
            ...
            <div className="row-[3] lg:row-span-3 lg:col-[2] h-[80%] flex flex-col justify-center border-solid gap-10">
                ...
                {ProjectsCategory.map((category)=>{
                      return (
                          <Link href={category.url} key={category.id} className="relative inline-flex items-center justify-center grow px-10 py-3 overflow-hidden font-medium transition-all bg-white rounded hover:bg-blue-300 group text-xl">
                              <p>{category.label}</p>
                          </Link>
                      );
                  })}
